type ('a, 'b) filesSystem = FolderType of 'a| FileType of 'b;;

type file = File of string;;
type folder = Folder of string *(folder,file) filesSystem list;; 
type disk = Disk of char * (folder, file) filesSystem list;;

let pathF disk toFind =
...
(* let disk2 = Disk('B',[]);;
let disk1 = Disk ('C', [FolderType(Folder ("Program Files", [FolderType(Folder ("F1", [FileType(File("P2.ml"))]));FolderType(Folder("F2",[]));FileType(File("P3.ml"))]));FolderType(Folder ("Program Files2", [FolderType(Folder ("F1", [FileType(File("P2.ml"))]));FileType(File("P2.ml"));FolderType(Folder("F3",[FileType(File("P6.ml"))]))]));FileType(File("P1.ml"))]);; (*;FolderType(Folder ("Program Files3", [FolderType(Folder ("F1", [FileType(File("P31.ml"))]));FolderType(Folder("F2",[]));FileType(File("P1.ml"))])) *)
path disk1 "P1.ml";; *)
