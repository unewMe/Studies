...
let pathF (dsk,toFind) =
  let rec searchName list =
    ...
    | h :: t -> 
      (match h with
      | FolderType(Folder(name, _)) when name = toFind -> true
      | FileType(File(name)) when name = toFind -> true
      | _ -> searchName t)
    | _ -> false
  and searchHelper (filesAndFolders,path,name,depth) =
      match searchName filesAndFolders with
      | true -> Some((path ^ name ^ "/"), depth)
      | false -> 
...
[FolderType(Folder ("Program Files", 
  [FolderType(Folder ("F1", 
   ...
   FileType(File("P1.ml"))]
);; (*;FolderType(Folder ("Program Files3", [FolderType(Folder ("F1", [FileType(File("P31.ml"))]));FolderType(Folder("F2",[]));FileType(File("P1.ml"))])) *)
pathF (disk1,"F29");;
pathF (disk1,"P2.ml");;
pathF (disk1,"P132.ml");;
pathF (disk2,"");;
