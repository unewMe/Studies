...
let path disk toFind =
      ...
      | None -> 
        List.fold_left (fun acc x ->
          ...
          | FolderType(subFolder) -> 
            ...
            | Some(_, acc_depth) -> let result = search subFolder (path ^ name ^ "/") (depth + 1) in
              (match result with
  ...
  | Disk(name, filesAndFolders) -> 
    let result = search (Folder((String.make 1 name)^":", filesAndFolders)) "" 0 in
    match result with
    | Some(path, _) -> Some(path)
    | None -> None
;;

let disk1 = Disk ('C', [FolderType(Folder ("Program Files", [FolderType(Folder ("F1", [FileType(File("P1.ml"))]));FileType(File("P2.ml"))]));FolderType(Folder ("Program Files2", [FolderType(Folder ("F1", [FileType(File("P2.ml"))]));FileType(File("P2.ml"))]));FileType(File("P8.ml"))]);;
path disk1 "P2.ml";;
